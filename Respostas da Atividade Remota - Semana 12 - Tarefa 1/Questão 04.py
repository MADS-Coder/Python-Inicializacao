...
def media_abertura(empresa, m, a):
    soma = cont = 0
    ordenado = sorted(empresa, key=itemgetter('ano', 'mes'))
    for p in ordenado:
...
def main():
    ...
    dados = carregar(nome)
    pm = media_abertura(dados, mes, ano)
    print(f'O preço médio de abertura em {mes}/{ano} foi {pm:.2f}')
